	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.allowedFirewallIPs != "" {
				err = firewall.AllowIPThroughFirewall(tt.allowedFirewallIPs, ViaAnyPort, &models.RuleSource{})
				require.NoError(t, err)
			}
...
func TestAttachRedirectorToCGroup_RedirectDNS(t *testing.T) {
	...
	err = firewall.AllowIPThroughFirewall("127.0.0.1", ViaAnyPort, &models.RuleSource{})
	require.NoError(t, err)
